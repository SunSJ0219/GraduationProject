...
class Net(torch.nn.Module):
    def __init__(self, in_chnl, hid_chnl):  # in_chnl = 3 : (x, y, e)
        ...
        for layer in range(1+3):  # 1 projection layer + x GIN layers
    ...
    def forward(self, x, edge_index, batch):
        ...
        for layer, layer_h in enumerate(hidden_rep):
            ...
            pool_over_layer += F.dropout(self.linears_prediction[layer](g_pool),
                                          0.5,
                                          training=self.training)

        return node_pool_over_layer, pool_over_layer
