...
class GNNPointer(nn.Module):
    def __init__(self, n_feature, n_hidden):
        super(GNNPointer, self).__init__()
        self.city_size = 0
        self.batch_size = 0
        self.dim = n_hidden
    ...
    def forward(self, x, X_all, mask, h=None, c=None, latent=None):
            ...
            if first_turn:
                ...
                h0 = h0.unsqueeze(0).contiguous() # contiguous()保证tensor中的元素连续存储
                c0 = c0.unsqueeze(0).contiguous()

                input_context = context.permute(1,0,2).contiguous() # permute对维度进行调换
